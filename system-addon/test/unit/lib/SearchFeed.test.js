...
describe("Search Feed", () => {
  ...
  it("shouldn't get the state if the content search is not initialized yet", () => {
    // pretend the content search is not yet initialized - so don't get the state either
    global.ContentSearch.initialized = false;
    sinon.stub(feed, "getState");
    feed.onAction({type: at.INIT});
    assert.notCalled(feed.getState);
    // reset the content search to be initialized
    global.ContentSearch.initialized = true;
  });
  ...
  it("should call services.obs.addObserver on INIT", () => {
    feed.onAction({type: at.INIT});
    assert.calledTwice(global.Services.obs.addObserver);
  });
  it("should call services.obs.removeObserver on UNINIT", () => {
    feed.onAction({type: at.UNINIT});
    assert.calledTwice(global.Services.obs.removeObserver);
  });
  ...
  it("should get the state if search content ready is fired", () => {
    sinon.stub(feed, "getState");
    feed.observe(null, "content-search-initialized", null);
    assert.calledOnce(feed.getState);
  });
});
